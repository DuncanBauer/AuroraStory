...
namespace util
{
    class AURORA_SERVER_API MongoDb
    {
    public:
        MongoDb() = delete;
        ...
        static inline InsertOneResult autoRegisterAccount(const std::string& username, const std::string& passwordHash, const std::string& ip)
        {
            ...
            try
            {
                ...
                auto doc = bsoncxx::builder::stream::document{}
                    ...
                    << "gm_level" << 0
                    << "logged_in" << 0
                    << "birthday" << static_cast<i64>(util::TimeTool::dateToEpochSeconds(1990, 01, 01)) // MongoDb only supports signed 64 bit int, so we need to cast it back to unsigned when reading
                    << "created_at" << static_cast<i64>(util::TimeTool::getEpochSeconds())              // MongoDb only supports signed 64 bit int, so we need to cast it back to unsigned when reading
                    << "last_login" << 0
                    << "last_ip" << ip
                    << "banned" << 0
                    << "nx" << 0
