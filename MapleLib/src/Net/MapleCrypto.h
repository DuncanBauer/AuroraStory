...
namespace Net
{
    class AURORA_MAPLE_API PacketParser
    {
        ...
        public:
            ...
            static inline void writePos(ByteBuffer* _buffer, size_t x, size_t y)
            {
    ...
    namespace Crypto
    {
        ...
        {
        public:
            MapleCrypto(short mapleVersion, byte* iv, bool cryptFlag)
            {
                m_IV = iv;
            ...
            static byte* getNewIV(byte* _oldIv)
            {
            ...
            // PACKET VERIFICATION
            byte* getHeaderToClient(int _size)
            {
              byte header[4] = { 0 };
            ...
            byte* getHeaderToServer(int _size)
            {
              byte header[4] = { 0 };
              int a = m_IV[3] * 0x100 + m_IV[2];
              a = a ^ m_MapleVersion;
            ...
            bool checkPacketToServer(byte* _packet)
            {
            ...
            // AES ENCRYPTION
            void crypt(byte* _data, int length)
            {
            ...
            {
              const byte* key = Constants::getTrimmedUserKey();
              return AesCrypt(iv, key, data, cryptFlag);
            }

            // MAPLE CUSTOM ENCRYPTION
            template<typename T>
            static void MapleEncrypt(byte* data, T length)
            {
              size_t j;
              byte a;
              byte c;
            ...
            template<typename T>
            static void MapleDecrypt(byte* data, T length)
            {
              size_t j;
              byte a;
              byte b;
        ...
        private:
          ...
          // Bytewise operations
          static void shuffle(byte inputByte, byte* start)
          {
            byte a = start[1];
            byte b = a;
            unsigned int c, d;
          ...
          static void multiplyBytes(byte* input, byte* output, int count, int mult)
          {
          ...
          // AES ENCRYPTION
          static std::string AesCrypt(const byte* _iv, const byte* _key, const byte* data, bool cryptFlag)
            {
                CryptoPP::AutoSeededRandomPool randPool;
                CryptoPP::HexEncoder encoder(new CryptoPP::FileSink(std::cout));

                CryptoPP::SecByteBlock key(_key, CryptoPP::AES::MAX_KEYLENGTH);
                CryptoPP::SecByteBlock iv(_iv, CryptoPP::AES::BLOCKSIZE);
                ...
                try
                {
                    if (cryptFlag)
                    {
                        ...
                        CryptoPP::StringSource s(data, true, new CryptoPP::StreamTransformationFilter(e, new CryptoPP::StringSink(retVal)));
                    }
                    else
                    {
                        CryptoPP::OFB_Mode<CryptoPP::AES>::Decryption d;
                        d.SetKeyWithIV(key, key.size(), iv);
                        CryptoPP::StringSource s(data, true, new CryptoPP::StreamTransformationFilter(d, new CryptoPP::StringSink(retVal)));
                    }
                }
        ...
        namespace Constants
        {
          ...
          static byte* getTrimmedUserKey() 
          {
            ...
            {
              key[i] = DEFAULT_MAPLESTORY_USER_KEY[i];
            }
            return key;
          }
