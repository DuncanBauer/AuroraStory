#pragma once

// Define dllexport and dllimport for Windows
#ifdef PA_PLATFORM_WINDOWS
    #ifdef PA_DYNAMIC
        #ifdef PA_MAPLE_BUILD_DLL
            #define AURORA_MAPLE_API _declspec(dllexport)
        #else
            #define AURORA_MAPLE_API _declspec(dllimport)
        #endif
    #else
        #define AURORA_MAPLE_API 
    #endif
#else
  #error PROJECT AURORA ONLY SUPPORTS WINDOWS!
#endif
#pragma endregion

#define AURORA_MAPLE_LIB_VERSION "0.0.1 ALPHA"
#define AURORA_MAPLE_LIB_VERSION_NUM 00001

// Includes
#include <vector>

// Constants
namespace Constants
{
    // Crypto
    const short MAPLE_VERSION = 83;
    const int IV_LENGTH = 16;
    const int INCREASE_PACKET_BUFFER_SIZE = 100;

    const unsigned char DEFAULT_AES_KEY_VALUE[] = {
        0xC6, 0x50, 0x53, 0xF2, 0xA8, 0x42, 0x9D, 0x7F, 0x77, 0x09, 0x1D, 0x26, 0x42, 0x53, 0x88, 0x7C
    };

    const unsigned char MAPLESTORY_AES_KEY[] = {
        0x13, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x00, 0xB4, 0x00, 0x00, 0x00,
        0x1B, 0x00, 0x00, 0x00, 0x0F, 0x00, 0x00, 0x00, 0x33, 0x00, 0x00, 0x00, 0x52, 0x00, 0x00, 0x00
    };
    const unsigned char DEFAULT_MAPLESTORY_USER_KEY[] = {
        
    };

    // NET ONLY
    const unsigned char BYTE_SHUFFLE[] = {
        
    };

    // WZ FILES ONLY
    const size_t WZ_FILE_OFFSET = 0x581C3F6D; // For WZ files
    const unsigned char WZ_GMS_IV[] = { 0x4D, 0x23, 0xC7, 0x2B }; // For WZ files
    const unsigned char WZ_MSEA_IV[] = { 0xB9, 0x7D, 0x63, 0xE9 }; // For WZ files. Used for kms as well?

    // NX FILES ONLY
}